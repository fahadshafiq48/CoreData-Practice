...
class ZoomAndSnapFlowLayout: UICollectionViewFlowLayout {
    ...
    var collectionSize: CGSize = CGSize(width: 150, height: 150)
    var centeredIndexPath: IndexPath? {
        didSet {
            if let index = centeredIndexPath {
                delegate?.cellIndexPathAtCentre(indexPath: index)
            }
        }
    }
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        if let centerCellIndexPath: IndexPath  = collectionView.centerCellIndexPath {
            ...
            delegate?.cellIndexPathAtCentre(indexPath: centerCellIndexPath)
        }
